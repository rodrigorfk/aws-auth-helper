...
function assumeRole (cli, profileName, token) {
  ...
  const params = {};
  if (awsConfig.source_profile) {
    params.credentials = new AWS.SharedIniFileCredentials({ profile: awsConfig.source_profile });
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    getMFAToken(cli).then((token) => {
      assumeRole(cli, profileName, token).then(response => {
        ...
        cli.stdout.write(`AWS_ACCESS_KEY_ID=${credentials.AccessKeyId} AWS_SECRET_ACCESS_KEY=${credentials.SecretAccessKey} AWS_SESSION_TOKEN=${credentials.SessionToken} AWS_PROFILE=`);
        resolve(credentials);
      }).catch(reject);
    }).catch(reject);
  });
